...
app.todo.push(function (env) {
	 ...
	 * @param cfg {Object} 配置
	 */
	env.Stage = function (id, cfg) {
		this.id = id || ("stage-" + env.util.rndStr());
		this.cfg = cfg || {};
	...
	env.Stage.prototype = {
		...
		start: function () {
			this.state = 1;
			env.util.each(this.acts, function (obj) {
				obj.start();
			});
		},
		pause: function () {
...
app.todo.push(function (env) {
	...
	env.Act = function (stage, id) {
		this.stage = stage;
		this.id = id || ("act-" + env.util.rndStr());
	...
	env.Act.prototype = {
		...
		start: function () {
			...
			env.util.each(this.scenes, function (obj) {
				obj.start();
			});
		},
		pause: function () {
...
app.todo.push(function (env) {
	...
	env.Scene = function (act, id) {
		this.act = act;
		this.stage = act.stage;
		this.is_gameover = false;
		this.id = id || ("scene-" + env.util.rndStr());
		/*
		 * state 有以下几种状态：
		 * 0: 等待中
		 * 1: 运行中
	...
	env.Scene.prototype = {
		...
		clear: function () {
			...
			env.util.shift(this._step_elements, function (obj) {
				env.util.shift(obj, function (obj2) {
					// element
					//delete this.scene;
					obj2.del();
//				delete this;
...
//			delete this;
			...
			env.util.shift(this._render_elements, function (obj) {
				env.util.shift(obj, function (obj2) {
					// element
					//delete this.scene;
					obj2.del();
...
//		delete this;
		...
		step: function () {
			...
			for (i = 0; i < 3; i ++) {
				...
				env.util.shift(level_elements, function (obj) {
					if (obj.valid) {
						if (!obj.is_paused)
							obj.step();
						a.push(obj);
		...
		render: function () {
			...
			if (this.is_gameover) {
				this.panel.gameover.show();
			}
		},
		addElement: function (el, step_level, render_level) {
			//env.log([step_level, render_level]);
