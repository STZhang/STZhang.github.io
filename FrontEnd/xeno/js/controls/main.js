	...
	init: function (td_board, is_debug) {
		...
		var env = {
			...
			init: function (ob_board/*, ob_info*/) {
				this.obj_board = env.util.$e(ob_board);
				this.canvas = this.obj_board.getElementsByTagName("canvas")[0];
				//this.obj_info = env.util.$e(ob_info);
				if (!this.canvas.getContext) return; // 不支持 canvas
				this.ctx = this.canvas.getContext("2d");
				this.monster_type_count = env.getDefaultMonsterAttributes(); // 一共有多少种怪物
			...
			start: function () {
				...
				this.util.mix(this, this.defaultSettings());
				this.stage = new env.Stage("stage-main", env.getDefaultStageData("stage_main"));
			 ...
			 *
			 * 用例：
			 * 1、增加 100 万金钱：javascript:app.cheat="money+";void(0);
			 * 2、难度增倍：javascript:app.cheat="difficulty+";void(0);
			 * 3、难度减半：javascript:app.cheat="difficulty-";void(0);
			 * 4、生命值恢复：javascript:app.cheat="life+";void(0);
			 * 5、生命值降为最低：javascript:app.cheat="life-";void(0);
			 */
			...
			step: function () {
				...
				if (this.is_debug && app && app.cheat) {
					// 检查作弊代码
					this.checkCheat(app.cheat);
					app.cheat = "";
				}
...
//						env.log("FPS: " + this.fps + ", Step Time: " + step_time);
					this.step_time = step_time;
				}
				if (this.iframe % 2400 == 0) env.gc(); // 每隔一段时间自动回收垃圾
